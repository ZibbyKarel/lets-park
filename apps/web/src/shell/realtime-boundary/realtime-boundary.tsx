 ...
 * string; that is `libs/realtime-client`'s job and nothing here routes around
...
export function RealtimeBoundary({ url, children }: RealtimeBoundaryProps) {
  ...
  return (
    <RealtimeProvider
      ...
      //
      // `no-console` is enforced in `apps/api/**` and `libs/**` only
      // (`eslint.config.mjs`); `apps/web` has no logger of its own, and a
