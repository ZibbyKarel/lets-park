 ...
 * `libs/realtime-client` and bans it from `apps/**` and `libs/**` alike, spec
 * files included. That ban is right and this file does not weaken it — the
 * `libs/realtime-client` wrapper is a React lib (`scope:web`), so `apps/api`
 ...
 * `libs/realtime-client`'s own `offline-transport.ts` fixture set for the other
