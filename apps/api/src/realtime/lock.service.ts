 ...
 * (`libs/contract/src/realtime/commands.ts`): holding one books nothing, and a
 ...
 * **extends** it, and `libs/realtime-client`'s `useCellLock` re-requests the
...
export abstract class LockService {
   ...
   * each — the guarantee `libs/realtime-client` relies on when it says "a
