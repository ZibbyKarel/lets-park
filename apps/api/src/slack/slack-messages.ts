 ...
 *
 * ## Why it is not in `libs/i18n`
 *
 * `libs/i18n` is tagged `scope:web` and `apps/api` is `scope:api`;
 * `@nx/enforce-module-boundaries` refuses the dependency, and it is right to —
 * `libs/i18n` wraps `next-intl`, a React package with no business in a Nest
 ...
 * in `libs/i18n`, so the two surfaces read as one product.
 */
