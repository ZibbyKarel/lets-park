 ...
 *
 * `libs/api-client` drives an `RPCLink` whose base URL ends in `/rpc`, and oRPC's
 * RPC protocol appends the procedure's **key path** in the contract router
 * (`libs/api-client/src/lib/api-client.spec.ts` pins
 * `POST /rpc/admin/spot/list`). So the routing table is not a decision this app
 * gets to make: it is the shape of `contract` in `libs/contract`, and the
