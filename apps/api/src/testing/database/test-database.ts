 ...
 * and dropped after it, and the developer's seeded `lets_park` is never written
 * to at all.
 *
 ...
 * and in particular the developer's `lets_park` is not.
 */
export const TEST_DATABASE_PREFIX = 'lets_park_dbspec_';

/** Env var carrying the URL of the developer's own database, for teardown. */
export const ORIGINAL_DATABASE_URL = 'LETS_PARK_ORIGINAL_DATABASE_URL';

/** The repo root — five directories up from `apps/api/src/testing/database`. */
const WORKSPACE_ROOT = join(__dirname, '..', '..', '..', '..', '..');
const MIGRATIONS_DIR = join(WORKSPACE_ROOT, 'libs', 'database', 'prisma', 'migrations');
