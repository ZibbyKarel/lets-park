 ...
 * string; that is `libs/lets-park/realtime-client`'s job and nothing here routes around
 * it (`doc/decision/0060-*`).
 */

import type { ReactNode } from 'react';
import { useAccessTokenProvider, useSession } from '@lets-park/auth/client';
import { RealtimeProvider } from '@lets-park/realtime-client';
...
export function RealtimeBoundary({ url, children }: RealtimeBoundaryProps) {
  ...
  return (
    <RealtimeProvider
      ...
      //
      // `no-console` is enforced in `apps/lets-park/api/**` and `libs/**` only
      // (`eslint.config.mjs`); `apps/lets-park/web` has no logger of its own, and a
