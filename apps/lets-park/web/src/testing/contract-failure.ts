 ...
 * `apps/lets-park/api`'s `ContractExceptionFilter` owns on the other side. Nine
 ...
 * `apps/lets-park/web` may not import `@orpc/client` to hand-build an `ORPCError`, and one
 * built here would assert this file's idea of the shape rather than the
 * transport's.
 *
 * It lives in `apps/lets-park/web/src/testing/` rather than in
 ...
 * `apps/lets-park/web` can import it without a new entry point.
 *
