 ...
 *
 * Mirrors `RPC_ROUTE_PREFIX` in `apps/lets-park/api/src/orpc/rpc-route.ts`, which cannot
 * be imported from here (the module boundary keeps `apps/lets-park/web` out of
 * `apps/lets-park/api`, and rightly: a shared constant would make the *client* the
 ...
 * every procedure sits under `/api` (`apps/lets-park/api/src/health/health.controller.ts`,
 ...
 * `apps/lets-park/web/src/env.ts` validates it with `z.url()` before the server serves
 ...
 * process, since `apps/lets-park/web/src/env.ts` validates `NEXT_PUBLIC_API_URL` with
