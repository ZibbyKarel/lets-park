/**
 * Fail-fast environment schema for apps/lets-park/api.
 *
 ...
 * interval — see `apps/lets-park/api/src/slack/daily-summary.job.ts`.
 */
...
const apiEnvObjectSchema = z.object({
   ...
   *
   * 30 s, and the number is not free: `libs/lets-park/realtime-client` renews at
   ...
   * path, only a different value here — the rule `apps/lets-park/api/src/auth` follows for
   * `AUTH_OKTA_ISSUER`.
   */
