 ...
 *
 * `libs/lets-park/realtime-client` (Task 21) shipped first and holds guarantees the
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleInit
{
   ...
   *   `socket.active === false`. `libs/lets-park/realtime-client` reads exactly that
   ...
   * ESLint wrapper ban forbids in `apps/lets-park/api` specs as much as in its
  ...
  handleDisconnect(client: RealtimeServerSocket): void {
    // The guarantee `libs/lets-park/realtime-client` leans on twice: "a dropped socket
