 ...
 *   comment and `doc/decision/0113-*`: `apps/lets-park/api` cannot import
 *   `libs/lets-park/realtime-client` directly (a boundary decision, not an oversight),
 ...
 *   bundle from `/socket.io/socket.io.js`. `apps/lets-park/web` bundles `socket.io-client`
 ...
 * ```
 *
 * Nothing in `realtime.gateway.ts`, and nothing in `libs/lets-park/realtime-client`,
