 ...
 * (`libs/lets-park/contract/src/realtime/commands.ts`): holding one books nothing, and a
 ...
 * **extends** it, and `libs/lets-park/realtime-client`'s `useCellLock` re-requests the
...
export abstract class LockService {
   ...
   * each — the guarantee `libs/lets-park/realtime-client` relies on when it says "a
