 ...
 *
 * `apps/lets-park/api/src/auth`'s rule — "dev, e2e and production all run this exact same
 ...
 *
 * The scan used to cover `apps/lets-park/api/src/realtime` and nothing else — five files
 * in one directory, because `readdirSync` is not recursive — while quoting a
 * rule taken from `apps/lets-park/api/src/auth`, which is where an `if (isTest)` would
 * actually let an unauthenticated caller in and which was guarded nowhere. It
 * now walks all of `apps/lets-park/api/src`. The file keeps its name because four places
...
/** `apps/lets-park/api/src` — this file lives one level under it. */
...
describe('the API has no environment-dependent behaviour', () => {
  ...
  it.each(['JEST_WORKER_ID', 'isTest', 'isDev', 'skipAuth', 'bypass'])(
    'never mentions %s anywhere in apps/lets-park/api/src',
    (needle) => {
