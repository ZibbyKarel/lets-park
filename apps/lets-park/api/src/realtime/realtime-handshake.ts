 ...
 * gateway reads.
 *
 * One guarantee `libs/lets-park/realtime-client` (Task 21) holds and this file is
 ...
 * object, which exists before any event does. `libs/lets-park/realtime-client` makes the
...
export class RealtimeHandshakeAuthenticator {
  ...
  async authenticate(socket: RealtimeServerSocket): Promise<void> {
    ...
    if (!auth.success) {
      // `libs/lets-park/realtime-client` sends `{}` — an object with no `token` key at
