 ...
 * `licensePlate: null` and no preferred spot (`libs/lets-park/database/src/lib/
 ...
 * `E2.61`'s seeded `group` is `'SHARED'` (`libs/lets-park/database/src/lib/
...
/** Already in the seeded style (`libs/lets-park/database/src/lib/seed-data.ts`), so the
 ...
 * (`apps/lets-park/web/src/lot/spot-dialog/spot-dialog.tsx`) is presentational and
...
test('regenerating the calendar link changes the URL, revokes the old one, and the new one works', async ({
  ...
  try {
    ...
    // feedEntriesForToken` (`apps/lets-park/api/src/calendar/calendar.service.ts`)
