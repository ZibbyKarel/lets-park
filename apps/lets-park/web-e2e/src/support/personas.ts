 ...
 *
 * Each one is a **seeded** account (`libs/lets-park/database/src/lib/seed-data.ts`), and
 ...
 *
 * The values are re-stated rather than imported: `libs/lets-park/database` is tagged
 * `scope:api` and this app is `scope:web`, so the module boundary forbids the
 * import (see `apps/lets-park/web-e2e/src/support/global-setup.ts` for the same
 ...
 * `use.storageState`. It used to return the relative `apps/lets-park/web-e2e/.auth/…`,
 * and Nx runs the executor with the cwd set to the project root, so the files
 * were actually written to `apps/lets-park/web-e2e/apps/lets-park/web-e2e/.auth/`. The two sides
 ...
 * the context root, so neither `apps/lets-park/web-e2e/.auth/` nor `storage-state*.json`
