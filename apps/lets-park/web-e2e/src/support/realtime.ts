...
function tracer(label: string): (direction: string, text: string) => void {
  ...
  return (direction, text) => {
    ...
    //                `libs/lets-park/realtime-client` and is asserted by its
 ...
 *    and `libs/lets-park/realtime-client` does not override it; socket.io-client here is
 ...
 *    `'serviceWorker' in navigator === true`). Nothing in `apps/lets-park/web` opens a
 ...
 * coming up: the page's `useSession` resolving, `libs/lets-park/auth` handing over an
