 ...
 *
 * `apps/lets-park/api/src/spots/spots.service.ts` is worth having read before this file:
...
test('deleting a spot with a reservation is refused, and succeeds once it is cancelled', async ({
  ...
  // A reload is required to see it: `libs/lets-park/contract/src/realtime/events.ts`
