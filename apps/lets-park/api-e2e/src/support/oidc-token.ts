 ...
 * nothing in `apps/lets-park/api-e2e` ever passed the guard. A suite like that does not
 ...
 *    without an `email`. `apps/lets-park/web-e2e/src/support/personas.ts` supplies the
 *    same pair through the browser; see `doc/decision/0180-*`.
 */

import axios from 'axios';

/** A seeded identity, matching `libs/lets-park/database/src/lib/seed-data.ts`. */
...
/** What `apps/lets-park/web` asks for; see `OKTA_SCOPES`. */
