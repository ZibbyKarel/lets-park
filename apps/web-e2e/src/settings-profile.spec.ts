 ...
 * `licensePlate: null` and no preferred spot (`libs/database/src/lib/
 ...
 * `E2.61`'s seeded `group` is `'SHARED'` (`libs/database/src/lib/
...
/** Already in the seeded style (`libs/database/src/lib/seed-data.ts`), so the
...
test('regenerating the calendar link changes the URL, revokes the old one, and the new one works', async ({
  ...
  try {
    ...
    // feedEntriesForToken` (`apps/api/src/calendar/calendar.service.ts`)
