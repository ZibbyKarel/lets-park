 ...
 * 2. `libs/database/src/scripts/reset-e2e.ts` — clears the target month and
 *    widens the reservation window. See that file for what it does and does not
 *    touch.
 *
 * ## Why subprocesses rather than imports
 *
 * `libs/database` is `scope:api`; this app is `scope:web`. The module boundary
...
export default function globalSetup(): void {
  ...
  run('node', ['--require', '@swc-node/register', 'libs/database/src/scripts/reset-e2e.ts']);
}
