...
| `CAR_COLOR_PALETTE` carries domain context ("car") in its identifier, inside an otherwise domain-clean design-system tokens lib | `libs/design-system/src/tokens` (`car-palette.ts`) | A real but narrower instance of the "domain words leaking into the design system" class, pre-existing from Task 7 and disclosed rather than hidden behind a "0 hits" claim | Still open, recorded so it isn't forgotten |
