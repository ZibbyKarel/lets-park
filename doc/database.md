...
| `Reservation (userId, date)` UNIQUE                                   | one user has at most one reservation per day (guest reservations, with `userId` null, are exempt — a guest has no identity beyond `guestName`, so one guest name can be given every bay in the lot on one day; see `doc/decision/0303-*`) |
| `Reservation_holder_check` CHECK                                      | `("userId" IS NULL) <> ("guestName" IS NULL)` – a reservation names exactly one holder, a user or a guest, never neither and never both (`doc/decision/0303-*`)                                                                           |
