...
Nastavení env proměnných (včetně výchozích hodnot) je v `doc/prostredi.md`.
...
| strukturované logování | `apps/api/src/logging/logger.options.ts` | `nestjs-pino`, JSON, `doc/decision/0033-*` |
| health sondy | `apps/api/src/health/` | `@nestjs/terminus`, `doc/decision/0032-*` |
...
- **Tenhle výstup není JSON.** Pino se instaluje až v `main.ts` přes
  ...
  „Riziko" v `doc/decision/0033-*`.

Reprodukce bez Dockeru: přeložit (`npx nx build api`) a spustit `node dist/apps/api/main.js`
s neúplným prostředím – návod v `doc/prostredi.md`, sekce „Demonstrace fail-fast".
...
závislost projektu, důvody v `doc/decision/0033-*`.
...
Proč zrovna tohle rozdělení a proč timeout: `doc/decision/0032-*`.
...
`SPOT_ALREADY_RESERVED` (409), ne 500. Detaily a celá mapovací tabulka: `doc/decision/0029-*`,
tvary odpovědí `doc/decision/0030-*`.
