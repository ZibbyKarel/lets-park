...
`libs/contract/src/api/router.spec.ts`.
...
14. **`FormField` stays a generic render-prop; `libs/form` stays `type:util`, upholding Task 18's deviation from its literal brief** — `form-tag-1` (Task 18)
    Why: `doc/workspace.md` line 242 had pre-assigned `libs/form` to `type:util` before Task 18 existed, and ESLint itself rejected the design-system-primitives import the retagging alternative would have required.
...
26. **`type:contract` may depend on `@orpc/contract` (not `zod` alone), deliberately excluding all other `@orpc/*` packages** — `contract-tag-1` (Task 1, provisional; endorsed and made final at Task 1 review)
    Why: the brief's literal "zod only" wording contradicts plan.md, which puts the oRPC contract definition inside `libs/contract`; plan.md is the binding spec and wins over its own argument (the brief).
...
28. **Escape dismissal moves to one shared LIFO dismissable-layer stack, rather than patching per-overlay `document` listeners a third time** — `overlay-stack-1` (Task 8, round 3)
    ...
    Cost if wrong: a contained refactor inside `libs/design-system/primitives`, with the five components' public props unchanged.
...
30. **Record BASE immediately before dispatch, and state which merged tasks it does and doesn't contain** — `task-20-base-1` (Task 20)
    Why: Task 20's brief described `apps/api/src/auth` state that its actual BASE (pre-Task-11-merge) did not have — a dispatch composed from merged main can silently diverge from the BASE recorded before that merge landed.
...
31. **Overlay nesting is established at registration time via React context, not inferred from the DOM; outer focus traps pause while an inner layer is registered above them** — `overlay-stack-3` (Task 8, round 4)
    ...
    Cost if wrong: still contained inside `libs/design-system/primitives`, with the five components' public props unchanged.
...
33. **Task 12 implements the contract via `@orpc/server` plus one per-procedure Nest route delegating to a shared `RPCHandler`, not `@orpc/nest`'s `@Implement`** — `orpc-nest-1` (Task 12; provisional, then confirmed final against the package source)
    Why: `@Implement` serves only the OpenAPI protocol and throws without a `contract.route()`, which `libs/contract` has none of, while the already-merged `libs/api-client` speaks the RPC protocol — confirmed `@orpc/nest` also needs `express>=5` against this tree's `express@4`, making it harder than the brief assumed.
    Cost if wrong: large — would mean rewriting `apps/api`'s transport layer; guarded by a parity spec proven by mutation (a procedure with no route, an admin route with no guard).
...
41. **Dispatch Task 22 (design-system compounds) concurrently with Task 13 (backend) and the Task 21 fix round (realtime-client)** — `parallel-3` (Task 22 dispatch)
    Why: the user's explicit instruction to parallelize independent phases, and the three agents' file sets (`apps/api`, `libs/realtime-client`, `libs/design-system/compounds`) are disjoint, so the conflict risk the serial default guards against does not apply.
...
57. **Correct the local `.env` files (still carrying `AUTH_OKTA_AUDIENCE=api://default`) to match the already-correct committed `.env.example`** — `t23-env-1` (Task 23)
    Why: the implementer refused to loosen validation, edit `.env`, or touch `apps/api` to work around the mismatch, running its own API on a spare port instead — the right call, but the local files were still simply wrong.
...
80. **Reject the claim that a `web:typecheck` failure predates Task 24 and is out-of-scope cross-cutting work; the branch owns it** — `t24-typecheck-not-preexisting` (Task 24 fix round 1; confirmed — an `apps/web/tsconfig.json` `exclude` glob missing `.spec.tsx`/`.test.tsx` let a new spec trigger a dual-declaration `@tanstack/query-core` hazard)
...
82. **The re-review must probe Task 24's round-2 typecheck fix (widening the `exclude` glob) by injecting a deliberate type error, not accept it on reading** — `t24-exclude-could-hide-errors` (Task 24 fix round 2; the probe fired and the answer was the bad one — round 2 had silently emptied the entire `apps/web` spec typecheck program to 0 of 18 files)
...
83. **Dispatch a prose-only fix for decision record `0126`, which is right in its conclusion (`module: esnext` is safe) but cites the wrong evidence (`apps/web/.swcrc`, which Jest never consults)** — `t24-adr-correction` (Task 24)
...
91. **Route to a fix round rather than accept the composite publisher's comment claiming isolation "for any delegate, present or future," when the isolation is measured to be synchronous-only** — `t16-async-delegate-hole` (Task 16 integration review)
    Why: an async delegate's rejection is not caught by `forward`'s `try`, and `apps/api` installs no `unhandledRejection` handler, so such a delegate would kill the process after `COMMIT`, on the request's way out — precisely the scenario the seam exists to prevent.
...
93. **Dispatch Task 28 (e2e) off main while Task 16 (Slack/scheduled jobs) is still in re-review, rather than serialising** — `t28-parallel-with-16` (Task 28 dispatch)
    Why: the two share no file surface — e2e adds `apps/web-e2e`/`apps/api-e2e` specs and `doc/testing.md`; Task 16 touches `apps/api/src/slack` and the publisher composite.
...
100. **Dispatch Tasks 32 and 33 as parallel implementers in separate worktrees, against the skill's default of never running implementers concurrently** — `t32-33-parallel` (Tasks 32/33 dispatch)
     Why: the two defects live in disjoint subsystems (realtime-client/apps/api realtime vs. apps/web auth), the project has run parallel branches throughout at the user's explicit request, and serialising two investigation-heavy tasks would roughly double the remaining wall clock.
