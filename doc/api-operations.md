...
This document describes the **operational baseline of `apps/api`**: what happens at
...
| --- | --- | --- |
| fail-fast env validation | `apps/api/src/env.ts` | Zod, runs inside `ConfigModule.forRoot({ validate })` |
| structured logging | `apps/api/src/logging/logger.options.ts` | `nestjs-pino`, JSON, `doc/decision/0036-*` |
| health probes | `apps/api/src/health/` | `@nestjs/terminus`, `doc/decision/0035-*` |
| graceful shutdown | `apps/api/src/shutdown/` | `GracefulShutdownService` |
| global error filter | `apps/api/src/common/filters/` | `doc/decision/0032-*`, `doc/decision/0033-*` |
| rate limiting | `apps/api/src/common/throttling/` | `@nestjs/throttler`, `doc/decision/0034-*` |
| helmet, CORS, prefix, body limit | `apps/api/src/configure-app.ts` | called by both `main.ts` and the tests |
...
into `apps/api/src/configure-app.ts`, so that `main.ts` and the tests call it through the
**same function**. `apps/api/src/app/http-pipeline.spec.ts` then starts a real Nest server
...
`node dist/apps/api/main.js` with an incomplete environment – instructions in
...
the built application – `apps/api/src/app/http-pipeline.spec.ts`):
