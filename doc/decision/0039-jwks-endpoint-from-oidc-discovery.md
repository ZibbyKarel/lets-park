# 0039 – The JWKS endpoint comes from OIDC discovery, and a cache miss never becomes a bypass
...
The test issuer in `apps/api/src/auth/testing/oidc-test-issuer.ts` publishes at `/jwks`
...
`doc/decision/0038-*` for the availability cost that carries.
