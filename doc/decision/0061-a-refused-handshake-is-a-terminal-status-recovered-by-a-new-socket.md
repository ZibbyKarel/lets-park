...
> `libs/auth`'s rotation has not run yet and the token is stale. The gateway refuses.
> `socket.io-client` destroys the socket. `libs/auth` refreshes the token perfectly well a
...
`libs/realtime-client/src/lib/connection.spec.tsx`, `describe('a refused handshake')` — five
...
gateway (Task 15), so the numbers are sized against `libs/auth`'s rotation rather than measured
