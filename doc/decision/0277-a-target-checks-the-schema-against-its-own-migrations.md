...
`nx run database:check-schema` (`libs/database/project.json`), four commands in
order:

```
prisma migrate status
prisma migrate diff --from-config-datasource \
  --to-schema libs/database/prisma/schema.prisma --exit-code
prisma generate
git diff --exit-code -- libs/database/src/generated
```
...
- **Nothing verified `schema.prisma`.** Both suites that call themselves parity
  ...
  - `schema-contract-parity.spec.ts` compares the **committed generated Prisma
    ...
    reference from `nx.json`, `package.json`, `.github/`, `apps/api/project.json`
...
- `git diff --exit-code -- libs/database/src/generated` is the honest form of
...
- `database:prisma-generate`'s `inputs` were `["{projectRoot}/prisma/schema.prisma"]`,
  ...
  with that list, touching `libs/database/README.md` still produced a cache hit;
