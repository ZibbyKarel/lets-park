...
The global filter (`apps/api/src/common/filters/contract-exception.filter.ts`) returns
...
real HTTP request exposed it. That's why `apps/api/src/app/http-pipeline.spec.ts` exists,
