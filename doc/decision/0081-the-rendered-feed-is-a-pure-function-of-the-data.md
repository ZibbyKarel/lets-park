...
- `libs/calendar-export/src/lib/reservation-calendar.ts` — no clock, no calendar
  `timezone`, `stamp: new Date(entry.createdAt)`.
- `libs/contract/src/api/ics.ts` — `createdAt` is part of `icsCalendarEntrySchema`
  for this reason.
- `apps/api/src/calendar/calendar.controller.ts` — `@Res({ passthrough: true })`
  and a returned string, so `res.send()` runs.
- `apps/api/src/calendar/calendar.service.ts` — `orderBy: [{ date: 'asc' }, { id:
