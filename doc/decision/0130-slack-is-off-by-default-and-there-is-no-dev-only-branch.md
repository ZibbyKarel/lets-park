...
- `apps/api/src/env.ts` — `SLACK_ENABLED` (enum, default `false`) and the
  `superRefine` that requires the token and channel when it is on.
- `apps/api/src/slack/slack-client.service.ts` — the one gate.
- `apps/api/src/slack/slack.config.ts` — `target` is present exactly when Slack
  is on, so the compiler carries the invariant instead of a `?? ''`.
- `apps/api/src/slack/slack-client.service.spec.ts` — disabled sends nothing at
