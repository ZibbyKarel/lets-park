...
`apps/api-e2e/src/support/oidc-token.ts` fetches an access token from the mock
OIDC issuer through the ordinary authorization-code flow, over plain HTTP.
`apps/api-e2e/src/api/auth.spec.ts` opens with three tests that use it:
...
`apps/api-e2e/project.json` no longer passes `--passWithNoTests`.
...
- **Two measured facts about the mock issuer are handled explicitly**, and both
  ...
  2. the default token carries exactly `aud, azp, exp, iat, iss, jti, nbf, sub,
     ...
     *Optional claims JSON* field, the same way
     `apps/web-e2e/src/support/personas.ts` does through the browser
