...
`apps/api/Dockerfile` has a fourth stage, `migrator`, whose whole job is
...
- **`migrate deploy`, never `migrate dev` and never `migrate reset`.** Deploy
  applies what is already in `libs/database/prisma/migrations` and generates
  nothing. `reset` destroys data and has no business in an image at all.
- **It seeds nothing.** `libs/database/src/scripts/seed.ts` says "Development
...
- `apps/api/Dockerfile`, stage `migrator`: `prisma@7.10.0` and `dotenv@17.4.2`
  pinned to the workspace lockfile's resolutions, `prisma.config.ts` and
  `libs/database/prisma` copied so the config's
