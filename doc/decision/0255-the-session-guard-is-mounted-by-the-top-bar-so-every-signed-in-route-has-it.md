...
- `apps/web/src/shell/app-top-bar.tsx` — `useRequireAuth()` in place of
  `useSession()`; the hook returns the same session, so the rest of the file is
  unchanged.
- `apps/web/src/app/(app)/layout.tsx` — the header states where the guard is
  and names the test that holds it.
- `apps/web/src/app/(app)/layout.spec.tsx` — new. Renders the real `AppLayout`,
...
- **The guard now runs on every signed-in screen, including during a slow
  ...
  asserted.
- **Mocking `next-auth/react` in `apps/web` is a reach past the wrapper, in a
