# 0040 – JIT provisioning matches on `oktaId`, falls back to `email`, and lets the database settle races
...
Then: `active === false` → `DomainError('FORBIDDEN')` (see `doc/decision/0037-*`).
...
`apps/web` renders `FORBIDDEN` as "your account is not allowed", which would be a plainly
...
`libs/database/prisma/schema.prisma` and verified only in CI. The retry logic, the catch and
