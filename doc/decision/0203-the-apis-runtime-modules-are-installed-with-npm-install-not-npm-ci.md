...
`apps/api/Dockerfile`'s `runtime-deps` stage runs

```
npm install --omit=dev --ignore-scripts --no-audit --no-fund
```

against the `package.json` webpack generates into `dist/apps/api`, and ignores
...
- **Nx's own targets for this cannot run here.** `api:prune` /
  `api:prune-lockfile` exist in `apps/api/project.json`, and both fail with
  `apps/api/package.json does not exist` — `@nx/js:prune-lockfile` expects a
...
- `apps/api/Dockerfile`, stage `runtime-deps`. `--ignore-scripts` because
...
- The `prune`, `prune-lockfile` and `copy-workspace-modules` targets in
  `apps/api/project.json` are left as they are: they are unused, and fixing
...
- **A transitive dependency can move between two builds of the same commit.**
  ...
  lockfile committed for `dist/apps/api` — not a hand-edited copy of a broken
  one.
