# 0041 – The token endpoint and its client-authentication method come from OIDC discovery

**Date:** 2026-09-02 · **Status:** accepted · **Task:** 20 (`libs/auth`)

## What

`libs/auth`'s refresh token rotation does not have a configured token endpoint. Given
...
single Okta URL in `.env.example`, shared by `apps/web` and `apps/api`, and the API's JWKS
...
`libs/auth/src/lib/refresh.ts`. The cache is a `let` inside the returned closure rather than a
