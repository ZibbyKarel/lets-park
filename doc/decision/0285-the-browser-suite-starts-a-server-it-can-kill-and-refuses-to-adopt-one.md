...
`apps/web-e2e/playwright.config.mts`'s web `webServer` entry changed in three
ways, and `apps/web-e2e/project.json` in one:

- The command is `npx next start --port 4200` with `cwd` set to `apps/web`,
...
`next build` writes a fresh random id to `apps/web/.next/BUILD_ID`, and a
...
- **The web server no longer inherits Nx's environment**, since Nx no longer
  starts it. In practice `next start` loads `apps/web/.env` itself and
