# 0044 – A failed refresh drops the token and signs the user out, rather than retrying or serving a stale bearer
...
`markRefreshFailed` in `libs/auth/src/lib/config.ts` builds the new token by destructuring the
