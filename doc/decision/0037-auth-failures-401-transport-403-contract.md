# 0037 – Authentication failure is a transport 401; a deactivated user is the contract's FORBIDDEN
...
builder in `libs/contract/src/api/errors.ts`, whose own comment says why: "a deactivated
...
"your credentials are missing or stale, go and get new ones" — so `apps/web` would send the
...
see an explanation. 403 with `code: 'FORBIDDEN'` is a code `libs/i18n` can key Czech copy
...
in reverse. It would also force `apps/web` to handle a code that can never appear in a
