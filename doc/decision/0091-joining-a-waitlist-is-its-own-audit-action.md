...
**Date:** 2026-09-02 · **Status:** accepted · **Affects:** `libs/contract`, `libs/database`,
`apps/api/src/reservations/` · **Follows on from:** `doc/decision/0016-*`, `doc/decision/0059-*`
...
- `libs/contract/src/schemas/entities.ts` — the member, last in the tuple; `entities.spec.ts`
  asserts the exact membership, so schema and contract cannot drift apart silently.
- `libs/database/prisma/schema.prisma` + the migration above. `ADD VALUE IF NOT EXISTS` keeps the
