...
`apps/api/Dockerfile` and `apps/web/Dockerfile` are multi-stage builds on
...
- **Non-root is the floor, not the ceiling.** `USER node` alone still leaves the
  ...
  mean. The one exception is `apps/web/.next/cache`, which Next writes to at
...
- **Readiness, not liveness, is what a compose `HEALTHCHECK` should report.**
  ...
  (`apps/api/src/health/health.controller.ts`, `doc/decision/0035-*`).
...
- `apps/api/Dockerfile` — stages `builder` → `runtime-deps` → `runner`, plus a
  separate `migrator` (`doc/decision/0204-*`). `HEALTHCHECK` on
  `http://127.0.0.1:${PORT}/health/ready`.
- `apps/web/Dockerfile` — stages `builder` → `runner`. `HEALTHCHECK` on
