...
`webServer[0].env` block in `apps/web-e2e/playwright.config.mts` setting
...
- **Setting them in `apps/api/project.json`'s `serve` target** would give every
...
- `apps/web-e2e/playwright.config.mts` — `webServer[0]` has no `env` block, and
  a comment saying why one would not work.
- `doc/testing.md` — the symptom, and the two remedies.
- Nothing in `apps/api` changes, and nothing ever did.
...
- **The suite cannot notice if the throttler broke.** It never could — no
  scenario asserts a `429`. The throttler's coverage is in `apps/api`'s unit
