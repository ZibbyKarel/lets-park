...
`libs/realtime-client` branches on exactly that field (`doc/decision/0061-*`). A refusal
...
`libs/realtime-client` makes the same call with its local `RealtimeHandshakeAuth`.
...
`libs/realtime-client` retries all of them three times. That is the deliberate trade
