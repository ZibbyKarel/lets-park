...
> `apps/web/src/auth.ts` calls `createAuth()` at module scope, so the refresher
...
confirmed the shape independently from the committed `apps/web/.next` — three
...
**How it reaches a user.** The proxy (`apps/web/src/proxy.ts` → `auth()`) and the
...
`libs/auth/src/lib/refresh.ts` — `TokenRefreshState`, `sharedRefreshStates()`,
...
`libs/auth/src/lib/config.ts` — passes `state: sharedRefreshState(...)`, and its
