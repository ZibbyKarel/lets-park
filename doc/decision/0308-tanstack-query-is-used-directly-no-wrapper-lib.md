...
Query in this workspace (`apps/web`), so a wrapper lib bought no swappability
...
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `createApiQueryUtils` (the oRPC ↔ TanStack Query bridge, built on `@orpc/tanstack-query`) | `libs/api-client/src/lib/api-query.ts`, exported from `@lets-park/api-client`        | `libs/api-client` already owns the oRPC client and already allow-lists `@orpc/tanstack-query` as a `type:util` companion package (same precedent as `@hookform/resolvers`); the bridge is oRPC-shaped, not TanStack-Query-shaped |
| `createQueryClient` + the retry policy (`shouldRetryQuery`, `MAX_QUERY_RETRIES`)           | `apps/web/src/shell/query/query-client.ts` (+ `retry.ts`)                            | single consumer, app-level policy — `apps/web/src/shell/` already hosts other single-consumer app utilities (e.g. `api-provider/`)                                                                                               |
...
real `QueryClient` are in `libs/api-client` (`api-query.spec.ts`); tests that
...
real stack" test — live in `apps/web/src/shell/query/` (`query-client.spec.ts`,
`query-integration.spec.tsx`), because only `apps/web`'s `tsconfig.json` uses
...
tests total in `apps/web` (up from 738 before this branch — 5 more from the
