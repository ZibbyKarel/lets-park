# 0045 – `AuthProvider` polls the session, because rotation only happens when the session is read
...
`REFRESH_SKEW_SECONDS`, which lives in this lib. Leaving it to `apps/web` would put half of
...
`SESSION_REFETCH_SECONDS` in `libs/auth/src/lib/client.tsx`, exported so the number is
citable.
