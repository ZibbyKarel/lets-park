...
Next.js's request interceptor (`apps/web/src/proxy.ts`, Node runtime) sees a
...
which `apps/web/src/auth.ts` otherwise deliberately prevents — it passes the
...
- `libs/auth` and `apps/web/src/auth.ts` are unchanged. `trustHost` was already
  stated as `true` in `libs/auth/src/lib/config.ts` and is not affected by this
  variable's presence.
