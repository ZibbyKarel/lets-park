...
once, in `libs/contract/src/realtime/socket-path.ts`, and exported from
`@lets-park/contract/realtime`. `apps/api/src/realtime/realtime-io.adapter.ts` imports it rather
than declaring its own copy, and `libs/realtime-client/src/lib/socket.ts` derives
`DEFAULT_SOCKET_PATH` — the name existing callers already use — from the same import instead of a
second literal. `apps/api/src/realtime/testing/realtime-test-client.ts`, the hand-built test peer
...
`realtime-io.adapter.spec.ts` (new) and `libs/realtime-client/src/lib/socket.spec.ts` (extended)
...
`apps/api`'s suite reads it and nothing in `libs/realtime-client`'s suite reads the adapter's
value.
...
`apps/api` (`scope:api`) cannot import `libs/realtime-client` (`scope:web`) — verified by the
...
This is a change to `libs/contract`'s file set, which the original Task 15 report declined to
...
constant, not a realtime event — so `libs/contract`'s own contract-first guarantees (schemas,
...
- `apps/api/src/realtime/realtime-io.adapter.spec.ts` constructs a real `socket.io` `Server`
  through `RealtimeIoAdapter` and asserts `server.opts.path === SOCKET_IO_PATH` (imported from
  the contract).
- `libs/realtime-client/src/lib/socket.spec.ts` asserts `DEFAULT_SOCKET_PATH === SOCKET_IO_PATH`
  (same import).
- `apps/api/src/realtime/testing/realtime-test-client.ts` — the peer every other realtime spec in
  `apps/api` connects with — now builds its connection URL from the same constant, so a real path
...
A single test that imports both `apps/api`'s adapter and `libs/realtime-client` in one file is not
...
one-line edit in `libs/contract/src/realtime/socket-path.ts` rather than a coordinated edit across
