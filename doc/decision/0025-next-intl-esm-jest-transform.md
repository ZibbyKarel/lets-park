# 0025 – `next-intl` is ESM-only; Jest in `libs/i18n` must also transpile `@orpc`

**Date:** 2026-08-28 · **Status:** accepted · **Affects:** `libs/i18n/jest.config.cts`

## What

`libs/i18n` runs into two independent ESM-only libraries in the same project,
...
`libs/i18n` is a React lib (generator `@nx/react:lib`), so unlike
`libs/contract` it doesn't transpile via `ts-jest`, but via `babel-jest`
...
- **`transformIgnorePatterns`** must allow both groups of packages at once
  ...
  not just one.
- **`.mjs` must go through `babel-jest`**, not `ts-jest` — `libs/i18n` doesn't
  use `ts-jest` at all.
...
which jsdom (the default test environment for `libs/i18n`, needed for
...
- `libs/i18n/jest.config.cts`: `transformIgnorePatterns` lists both package
  ...
  (asset) rule.
- `libs/i18n/src/lib/errors.spec.ts`: the pragma comment
...
- `doc/decision/0020-*` itself says that once this fix is needed a third time,
  ...
  17 is only allowed to touch `libs/i18n/**`, so it stays local for now.
  Whoever eventually unifies it should unify both copies (`libs/contract` and
  `libs/i18n`) at once.

## Risk if this is wrong

The duplication risk is the same as in `doc/decision/0020-*`: the copies can
drift apart. A risk specific to `libs/i18n` is that ordering in `transform` —
