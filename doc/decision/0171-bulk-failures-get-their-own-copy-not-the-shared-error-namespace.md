...
**Date:** 2026-09-03 · **Status:** accepted · **Affects:** `libs/i18n`, `apps/web/src/lot/bulk-*`
...
- `bulk-view.spec.ts` builds every failure by driving a **real** `RPCLink` with a stubbed `fetch`
  (`apps/web` may not import `@orpc/client`), the same technique `screen-state.spec.tsx` uses, so
