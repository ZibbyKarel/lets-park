# 0043 – The access token crosses to the browser; the refresh token never does
...
**The browser genuinely needs the access token.** `libs/realtime-client` (Task 21) opens a
...
`projectSession` in `libs/auth/src/lib/config.ts` rebuilds `accessToken`/`error` from the
...
passed to `createAuth` as arguments, are validated in `apps/web/src/env.ts`, and carry no
