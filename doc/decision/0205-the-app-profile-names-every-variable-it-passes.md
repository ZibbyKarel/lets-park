...
- **List form, not map, and the difference is not cosmetic.** An entry with no
  ...
  `apps/api/src/env.ts` has a default that an empty string does not satisfy —
...
- **`${VAR:?…}` for required values was tried and had to be removed.** It reads
  ...
  *required* value is caught where it always was, by the app's own fail-fast
  schema at boot (`apps/api/src/env.ts`,
  `apps/web/src/instrumentation-node.ts`), which names the variable and exits.
...
are listed as bare pass-throughs even though `apps/api/src/slack` does not exist
