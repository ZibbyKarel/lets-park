...
- **A category is not data, it is a type.** `PARKING_GROUPS = ['IT', 'SHARED']`
  is a `const` tuple in `libs/shared-types/src/lib/domain-constants.ts`. Every
  layer is built on it: `libs/contract` derives `parkingGroupSchema` with
  `z.enum(PARKING_GROUPS)`, Prisma has it as a Postgres `enum` column, and
  `apps/api` filters and orders by it.
