# 0046 – `next-auth` is pinned to an exact v5 **beta**, and `latest` must never be taken
...
**v4 is not a fallback; it is a different library.** `libs/auth` is built on the v5 App Router
...
`npm install next-auth@5.0.0-beta.32 --save-exact`. `libs/auth` is the only importer
