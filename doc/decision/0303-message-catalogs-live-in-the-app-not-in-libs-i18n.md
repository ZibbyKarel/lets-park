# 0303 – Message catalogs live in the app, not in `libs/i18n`
...
```
apps/web/messages/cs.json      280 keys, the source of truth
apps/web/messages/en.json      the same 280 keys
apps/web/messages/messages.spec.ts   the parity guard over both
```

`libs/i18n` holds **no copy at all**. It kept `next-intl` — it is still the
only project in the workspace allowed to import the package — and gave up its
catalog: `libs/i18n/src/lib/messages.ts` is deleted, and the lib's three
consumers of it became locale-parameterised.

| Before, in `libs/i18n`                                                 | After                                                       |
| ---------------------------------------------------------------------- | ----------------------------------------------------------- |
...
(`apps/web/src/app/layout.tsx` → `resolve-locale.ts`, `load-messages.ts`) and
hands both down through `apps/web/src/app/providers.tsx` to `IntlProvider`.
The one Czech-only table `Intl` cannot produce, `MONTH_LOCATIVE_CS`, stays in
`libs/i18n/src/lib/dates.ts`: it is not copy a translator writes, it is
linguistic data the formatter needs.

## Why it could not be otherwise

`TODO.md` item 9 puts the JSON in `apps/web/messages`, which is also
`next-intl`'s own documented layout. That alone would be a preference. What
makes it structural is Nx: `libs/i18n` is a library tagged `type:util`,
...
`load-messages.ts`'s catalog map — no change in `libs/i18n` at all.
...
`apps/web/next-intl.d.ts` is the whole mechanism:
...
The file lives in `apps/web` and not in the lib for the same Nx reason as
everything above: `libs/i18n` may not read `apps/web/messages/cs.json`. And
...
1. `libs/i18n/src/lib/errors.spec.ts` used to iterate the contract's
...
2. The deleted TypeScript catalog declared
   ...
   works because `apps/web/tsconfig.spec.json` includes
...
- `apps/web/messages/messages.spec.ts` — key parity in **both** directions, no
...
- `libs/i18n`'s own six suites now test the lib as a locale-parameterised
