# 0030 – `FormField` is a generic render prop; `libs/form` does not import the design system

**Date:** 2026-08-28 · **Status:** accepted · **Task:** 18 (`libs/form`)

## What

`FormField` (`libs/form/src/lib/form-field.tsx`) doesn't accept a primitive's name or render
...
Consequence: **`libs/form` (the production source) does not import the design system at
all.** The only place `@lets-park/design-system-primitives` appears inside `libs/form` is a
...
primitives") would mean `libs/form` (tag `type:util`) depends on `design-system-primitives`
...
states that design-system composition lives "only in `apps/web`" (domain and non-domain
alike) — `libs/form` belongs there no more than `libs/i18n` does.
...
- **It adds no coupling** between the wrapper and the design system — Task 8
  ...
  specific set of primitives, every future primitive would require a change in `libs/form`,
...
- The test that proves the whole point of the wrapper (a form built from `@lets-park/form` +
  ...
  to live in `libs/form`, because Task 18 is only allowed to touch `libs/form/**`. This is
  solved with a separate `depConstraints` override just for `libs/form/**/*.spec.{ts,tsx}`
  ...
  files matching that glob, not for the rest of `libs/form` and not for any other `type:util`
  lib. Alongside it, `allowCircularSelfDependency: true`, because the demo test imports
  `@lets-park/form` via its own alias from inside `libs/form` (to prove the public API is
...
- Verified with **four** temporary probe files (deleted afterward, see `doc/workspace.md` on
  ...
  | --- | --- | --- | --- |
  | `import 'react-hook-form'` | `libs/form/src/lib/*.ts` | allowed | lint green |
  | `import 'react-hook-form'` | `libs/i18n/src/lib/*.ts` | forbidden | `no-restricted-imports … use @lets-park/form` |
  | `import '@lets-park/design-system-primitives'` | `libs/form/src/lib/*.spec.tsx` | allowed | lint green |
  | `import '@lets-park/design-system-primitives'` | `libs/form/src/lib/*.ts` (not `.spec.`) | forbidden | `type:util … can only depend on … "type:util", "type:contract"` |
  ...
  opened up `type:ui` for `libs/form` as a whole.

## Risk if this is wrong

If a future task (23–27, 31) genuinely needs `libs/form` itself to render specific
...
which would open the same path to every other wrapper (`libs/query`, `libs/api-client`, …),
