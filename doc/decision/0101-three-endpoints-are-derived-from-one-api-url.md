...
configured value itself**. `apps/web/src/api-url.ts` derives all three:
...
- `API_RPC_SEGMENT` mirrors `RPC_ROUTE_PREFIX` in
  `apps/api/src/orpc/rpc-route.ts` by value, not by import. The module boundary
  keeps `apps/web` out of `apps/api`, and rightly — a shared constant would
...
- **The `/rpc` segment is duplicated knowledge.** If the API ever moves its RPC
  ...
  say so. The alternative — publishing the segment from `libs/contract` — is a
