...
`libs/database/src/lib/seed-data.ts` no longer carries an `icsToken` field at
...
`libs/database/src/lib/ics-token.ts` mints one instead —
`randomBytes(32).toString('base64url')` — and `libs/database/src/lib/seed-writes.ts`
...
- **Those four values were published credentials.**
  `apps/api/src/calendar/calendar.controller.ts` is `@Public()` by design, and
...
- **"Dev only" was a convention, not a mechanism.** `seed.ts` checked that
  ...
  half of this fix.
- **`base64url`, not hex.** `apps/api/src/auth/auth-user.service.ts` mints a
...
- **`role` and `active` deliberately stay in `update`.** Those are the seed's to
  assert on every run, and `apps/web-e2e` depends on `inactive@example.com`
  being inactive when a suite starts. The distinction is that a credential
  belongs to whoever holds it and a role does not.

## How

- `libs/database/src/lib/ics-token.ts` and its spec: 32 bytes, base64url,
  never repeating.
- `libs/database/src/lib/seed-writes.ts` owns the `upsert` argument shape so
