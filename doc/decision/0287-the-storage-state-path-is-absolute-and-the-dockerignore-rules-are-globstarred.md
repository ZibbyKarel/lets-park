...
- `apps/web-e2e/src/support/personas.ts` — `storageStatePath()` returns
...
written to `apps/web-e2e/apps/web-e2e/.auth/`. Confirmed on disk. The two sides
...
context root. `apps/web-e2e/.auth/` named a directory that did not exist, and
...
/ctx/apps/web-e2e/apps/web-e2e/.auth/storage-state-user.json   ← NOT excluded
```

`.env`, `apps/web/.env`, a root-level `storage-state-*.json` and
`apps/web-e2e/.auth/…` were all excluded correctly. **Every rule in the file
...
- **The patterns.** `**/`-prefixed, for the same reason `**/.env` is — the
  ...
  bare `.env` shipped `apps/web/.env` into an image. A pattern that is right
...
are at `apps/web-e2e/.auth/`, and `apps/web-e2e/apps/` does not exist.
...
- **A stale `apps/web-e2e/apps/` directory** may exist in a working tree that
...
- **`workspaceRoot` walks up from `process.cwd()` for `nx.json`.** Every path
  ...
  `apps/web-e2e` — is inside the workspace, so it resolves. A `playwright test`
