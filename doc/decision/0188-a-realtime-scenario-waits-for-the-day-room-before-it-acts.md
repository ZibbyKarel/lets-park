...
`waitForDayRoom` (`apps/web-e2e/src/support/realtime.ts`) watches the page's own
...
- `apps/web-e2e/src/support/realtime.ts` — `recordDayRoomSubscriptions`,
  `subscribedDates`, `waitForDayRoom`, and the `E2E_TRACE_REALTIME` switch that
  found the real bug.
- `apps/web-e2e/src/support/fixtures.ts` — the recorder is attached in
  `pageFor`, before the page navigates.
- `apps/web-e2e/src/cell-lock.spec.ts` — both tests.
