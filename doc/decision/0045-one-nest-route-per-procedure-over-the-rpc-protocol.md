# 0045 – One Nest route per procedure over oRPC's RPC protocol, not `@orpc/nest`'s `@Implement`
...
`apps/api` serves the contract from Task 4 as **one `@Post()` Nest route per contract
procedure**, each delegating to a single shared `RPCHandler` from `@orpc/server`:

- `apps/api/src/orpc/rpc-route.ts` derives the path — `/api/rpc/<segment>/<segment>`, so
  `admin.spot.create` is `POST /api/rpc/admin/spot/create`.
- `apps/api/src/orpc/rpc-route-handler.ts` owns the one `RPCHandler`, mounted with
  `prefix: '/api/rpc'`, and turns an unmatched path into a `NotFoundException`.
- `apps/api/src/orpc/implementer.ts` is `implement(contract).$context<OrpcContext>()` plus the
...
`apps/api/src/orpc/orpc-route-parity.spec.ts` is what keeps this honest: it walks the contract
...
`libs/api-client` (Task 19, merged) builds its client with `RPCLink` from `@orpc/client/fetch`,
and its spec asserts the exact request an `apps/web` call produces:
...
3. deciding a REST method and path for each of the 20 procedures — information the contract does
   not carry, so it would have been invented in `apps/api` and duplicated in the client.
...
- Adding a procedure to the contract is **two** edits in `apps/api`: the oRPC implementation, and
