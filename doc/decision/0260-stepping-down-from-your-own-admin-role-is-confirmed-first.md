...
In `apps/lets-park/web/src/shell/admin/admin-users-screen/admin-users-screen.tsx`, switching the viewer's own
...
- `apps/lets-park/web/src/shell/admin/admin-users-screen/admin-users-screen.tsx` — one `useState`, one guard
...
- `libs/shared/i18n/src/lib/messages.ts` — three new keys in `admin`
  ...
  declarations. **This is the one file outside `apps/lets-park/web` this change touches**;
  it is an additive leaf in the `admin` namespace.
- `apps/lets-park/web/src/shell/admin/admin-users-screen/admin-users-screen.spec.tsx` — six tests: it asks
