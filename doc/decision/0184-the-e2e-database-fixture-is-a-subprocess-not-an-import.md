...
`apps/web-e2e`'s `globalSetup` prepares the database by running two commands:

```
npx prisma db seed
node --require @swc-node/register libs/database/src/scripts/reset-e2e.ts
```
...
- **The module boundary forbids the import, and is right to.** `libs/database`
  is tagged `scope:api`; `apps/web-e2e` is `scope:web`, which
...
- `apps/web-e2e/src/support/global-setup.ts` — `execFileSync` for both commands,
...
- `libs/database/src/scripts/reset-e2e.ts` — the reset itself.
- `libs/database/project.json` — `reset-e2e` target, for running it by hand.
