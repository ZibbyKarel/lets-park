...
The tile `<button>` in `apps/web/src/lot/lot-grid/lot-grid.tsx` labels itself with the
...
- `apps/web/src/lot/lot-grid/lot-grid.tsx` — `accessibleName` is built from the action
...
- `apps/web/src/lot/lot-grid/lot-grid.spec.tsx` — a new block, *"what a screen reader is
...
- `apps/web/src/lot/lot-screen/lot-screen.spec.tsx` — its tile queries were loosened to
...
- **`apps/web-e2e/src/support/lot-page.ts` still anchors its selector.**
