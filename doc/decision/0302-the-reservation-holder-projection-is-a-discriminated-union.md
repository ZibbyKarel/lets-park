...
`publicReservationSchema` (`libs/contract/src/schemas/entities.ts`) no longer
...
verification found exactly this set of breaks — `apps/api`'s mapper
(`prisma-mapping.ts`, `day-overview.service.ts`) and four call sites in
`apps/web` (`day-overview-cache.ts`, `lot-view.ts`,
...
projection change propagates correctly and nothing in `libs/contract` or
`libs/realtime-client` was missed.
