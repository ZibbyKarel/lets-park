...
`libs/contract/src/api/reservation-window.ts:31`.
...
`apps/web` is the client that would have to be checked against it. The fix round
that found this owns `libs/contract` and explicitly does not own `apps/`, and
...
defaulted), and if so, does `apps/web`'s admin form already send both? A
follow-up needs to change both sides in one commit.

## M-4 — a tautological assertion in `router.spec.ts`

`libs/contract/src/api/router.spec.ts:196-208`. The test *declares both window
