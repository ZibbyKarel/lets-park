...
`libs/shared-types/src/lib/czech-holidays.ts` computes the thirteen Czech public holidays
...
This app needs thirteen dates a year in one country. `libs/shared-types`' barrel is re-exported
wholesale as `@lets-park/i18n` (`doc/decision/0003-*`) and `apps/web/src/lot/lot-view.ts` calls
