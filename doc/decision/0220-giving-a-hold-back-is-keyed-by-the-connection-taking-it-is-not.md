...
Measured before the change, `apps/api/src/realtime/lock.service.spec.ts`:
...
- `apps/api/src/realtime/lock.service.ts` — the abstract signature, the
...
- `apps/api/src/realtime/realtime.gateway.ts` — `cellUnlock` passes
  `{ user: client.data.user, socketId: client.id }`.
- `apps/api/src/realtime/lock.service.spec.ts` — "refuses a superseded
...
- `apps/api/src/realtime/realtime.gateway.spec.ts` — the same claim over real
