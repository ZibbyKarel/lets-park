...
`apps/web/src/shell/admin/admin-errors.ts` holds a table from an **admin write**
...
- **A table can be checked against the contract; a chain of `if`s cannot.**
  ...
  `libs/contract` and asserts the table has a sentence for every code the
...
- **`FORBIDDEN` is spread into every row.** Every authenticated procedure
  inherits it (`authed` in `libs/contract/src/api/errors.ts`), and it is the one
