...
| --- | --- | --- |
| `REALTIME_LOCK_TTL_MS` | `apps/api/src/env.ts` | 30 000 ms |
| `CELL_LOCK_RENEW_FRACTION` | `libs/realtime-client` | 0.5 |
| `CELL_LOCK_ACK_TIMEOUT_MS` | `libs/realtime-client` | 5 000 ms |
| `CELL_LOCK_ACK_ATTEMPTS` | `libs/realtime-client` | 2 |
...
So that the specs can run the *same* gateway against a short TTL. `apps/api/src/realtime/**`
contains no `NODE_ENV` check, no test flag and no bypass; the only thing that differs between
a spec and production is this value — exactly the rule `apps/api/src/auth` follows for
