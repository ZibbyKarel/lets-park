...
`libs/i18n/src/lib/locale.ts`), and **no URL says which one you are reading**.
Every route this app serves is the same path in both languages: `/`, `/admin`,
`/login`, `/settings`.

The locale for one request is decided in `apps/web/src/app/layout.tsx`, through
`apps/web/src/i18n/resolve-locale.ts`, from exactly two inputs:
...
The decision function is `negotiateLocale` in `libs/i18n`, deliberately free of
`next-intl` so the client error boundary (`apps/web/src/app/global-error.tsx`,
...
(`apps/web/src/shell/top-bar/top-bar.tsx`, under a `nav.language` header). It
...
`apps/web/src/shell/top-bar/use-locale-switch.ts`. `path=/` because the choice
...
**1. `apps/web-e2e` asserts concrete paths, and a prefix would move all of
them.** The browser suite navigates and asserts `/`, `/login`, `/settings` and
`/admin` by name, through `apps/web-e2e/src/support/*`; `libs/auth`'s
...
- **The switch is a `router.refresh()`, not a navigation.** The locale is
  ...
  server render — verified end to end in `apps/web-e2e/src/locale.spec.ts`,
...
prefix, its request interceptor composed into `apps/web/src/proxy.ts` — which
already holds the session check, and which Next.js 16 named `proxy` rather than
`middleware` — `getRequestConfig` in `apps/web/src/i18n/request.ts`, and the
route tree moved under `app/[locale]/`.
That change **must also** rewrite `apps/web-e2e/src/support/*` and every
