# 0082 — The feed's Czech copy lives in `libs/calendar-export`, not in `libs/i18n`
...
`libs/calendar-export/src/lib/reservation-calendar.ts`, not entries in the
`libs/i18n` message catalog.

They stay **Czech**, like the rest of the UI (`doc/decision/0029-*`).

## Why

`libs/i18n` is tagged `scope:web`. `libs/calendar-export` and `apps/api` are
...
right: `libs/i18n` wraps `next-intl`, which is a React/Next package with no place
...
This is not a general licence to scatter Czech copy through `apps/api`. It
applies because the ICS feed is the **only** backend-rendered surface a user ever
reads — every other response is JSON that `apps/web` turns into words. If a
...
- `libs/calendar-export/src/lib/reservation-calendar.ts` — `ICS_CALENDAR_NAME`,
  `icsEventSummary`, `icsEventDescription`.
- `libs/calendar-export/src/lib/reservation-calendar.spec.ts` — asserts the
  Czech strings by reading them back through `ical.js`, including that the
  diacritics survive line folding.
- `apps/api/src/calendar/calendar-pipeline.spec.ts` — asserts they survive the
