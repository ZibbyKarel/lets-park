...
`apps/api/src/realtime/realtime-no-backdoor.spec.ts` now walks **all of
`apps/api/src`** recursively, skipping `**/testing/**` and `*.spec.ts`, instead
...
`apps/api/src/auth`'s own header, and `apps/api/src/auth` is where an
