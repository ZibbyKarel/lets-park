...
Every schema in `libs/contract` that narrows `userSchema` for an audience is a
...
Narrowing that is a separate question with a UI consequence (`apps/web`'s admin
...
`libs/contract/src/api/users.ts` exports `ADMIN_USER_FIELDS`, the allowlist, and
