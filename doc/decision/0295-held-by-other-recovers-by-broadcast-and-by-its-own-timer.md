...
`useCellLock` (`libs/realtime-client/src/lib/cell-lock.ts`) no longer treats `held-by-other` as
...
It reached no user: `apps/web` discards the return value (`lot-screen.tsx:132`) and drives tile
...
`apps/web` actually uses the hook. It was rejected for two reasons, both measured by grepping the
workspace rather than argued:

1. **Blast radius, almost all of it outside this task's file set.** `held-by-other` is load-bearing
   in prose that justifies *server* behaviour: `apps/api/src/realtime/lock.service.ts:176-177` and
...
- `apps/web` still discards the return value and still drives tile badges from `useCellLocks`.
