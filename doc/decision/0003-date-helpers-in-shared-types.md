# 0003 – Europe/Prague date logic lives in `libs/lets-park/shared-types`, not `libs/shared/i18n`
...
implemented in **`libs/lets-park/shared-types`**, already in Phase 1. `libs/shared/i18n` in Phase 4 only
...
implementation"), or `apps/lets-park/api` would drag in next-intl.

`plan.md` itself introduces `libs/lets-park/shared-types` for "domain types/constants not directly
tied to the contract" and explicitly allows the helper to live "in `libs/shared/i18n` **or**
`libs/lets-park/shared-types`" (§Contract-first, date semantics) – this decision just fixes which
of the two options applies.

## How

- `libs/lets-park/shared-types` has no runtime dependency on next-intl or on Zod.
...
- ESLint `no-restricted-imports` forbids importing `libs/shared/i18n` in `apps/lets-park/api`.
- `libs/shared/i18n` re-exports `libs/lets-park/shared-types`'s public API under the same names, so FE
