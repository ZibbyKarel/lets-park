...
`isBusinessDay` is a pure function in `libs/shared-types` and the contract can
...
- Schemas and both procedures: `libs/contract/src/api/bulk.ts`.
- The enums `BULK_DAY_OUTCOMES` and `BULK_UNAVAILABLE_REASONS`, and the
  constant `MAX_BULK_BOOKING_DAYS`, live in `libs/shared-types` (like every
  domain enum), so `libs/i18n` can also see them for the Czech copy.
