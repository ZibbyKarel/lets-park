# 0047 – A reservation-window change gets its own audit action

**Date:** 2026-09-02 · **Status:** accepted · **Follows on from:** `doc/decision/0026-*`

## What

`AUDIT_LOG_ACTIONS` in `libs/contract/src/schemas/entities.ts` and `enum AuditLogAction` in
`libs/database/prisma/schema.prisma` gained one member: **`RESERVATION_WINDOW_UPDATED`**.
...
- `libs/contract/src/schemas/entities.spec.ts` asserts the enum's exact membership, so the next
