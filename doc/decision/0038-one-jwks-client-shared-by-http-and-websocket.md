# 0038 – One JWKS client and one rule set, shared by the HTTP guard and the WebSocket handshake
...
Token verification lives in `JwksVerifierService` (`apps/api/src/auth/jwks-verifier.service.ts`),
