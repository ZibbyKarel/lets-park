...
**1.** `apps/api/src/app/app.controller.ts`, `app.service.ts` and their two specs
...
`apps/api-e2e/src/api/api.spec.ts`'s header already recorded the situation
...
- `apps/api/src/auth/auth-pipeline.spec.ts` used it as its "a route with no auth
...
- **`apps/api-e2e/src/api/api.spec.ts` still asserts 401 and will fail.** That
