...
Before the browsers start, `libs/database/src/scripts/reset-e2e.ts` sets the
...
- `libs/database/src/scripts/reset-e2e.ts` — deletes the month's reservations
  and queue entries, upserts the window settings, refuses to run with
  `NODE_ENV=production`.
- `libs/database/project.json` — `nx run database:reset-e2e` for running it by
  hand.
- `apps/web-e2e/src/support/global-setup.ts` — runs `prisma db seed` and then
  the reset script, as subprocesses (see `0184`).
- `apps/web-e2e/src/support/dates.ts` — `e2eBusinessDays()` and the
