...
stale token *across* a reconnect; it presents whatever `libs/auth`'s provider has at that moment.
...
- **The exposure is a room broadcast, and nothing else.** The four server → client events carry a
  ...
  step (`libs/contract/src/realtime/commands.ts`).
...
a client one: `libs/realtime-client` cannot enforce it, because the client is the party whose
token is in question.

## Where it is written down

`libs/realtime-client/src/lib/socket.ts`, in the "Why `auth` is a function" section — the exact
