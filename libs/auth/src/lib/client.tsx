 ...
 * code uses.
 *
 * Reached as `@lets-park/auth/client`, separate from the server entry point on
 * purpose. `@lets-park/auth` pulls in `next-auth`'s server runtime (route
 ...
 * can actually run — the same split `libs/contract` makes between its API and
 * realtime entry points.
 *
 * Like `QueryProvider` in `libs/query` and `IntlProvider` in `libs/i18n`, this
 ...
 * That combination is the whole reason `libs/api-client` takes a function
 * instead of a string.
 */
