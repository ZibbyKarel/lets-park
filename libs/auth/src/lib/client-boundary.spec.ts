 ...
 *
 * `@lets-park/auth/client` is meant to reach `next-auth/react` and nothing
 * else; `@lets-park/auth` is what reaches Auth.js's server runtime (route
 * handlers, `next/server`, the OAuth machinery). Keeping them apart is the
 * whole reason there are two entry points (`doc/decision/0042-*`), and it is
...
jest.mock('next-auth', () => {
  throw new Error(
    '@lets-park/auth/client reached "next-auth" — the server half. See doc/decision/0042-*.'
  );
});
jest.mock('next-auth/providers/okta', () => {
  throw new Error(
    '@lets-park/auth/client reached "next-auth/providers/okta" — the server half. See doc/decision/0042-*.'
  );
});
jest.mock('./config', () => {
  throw new Error(
    '@lets-park/auth/client reached "./config" — the server half. Move what it needed into src/lib/session.ts. See doc/decision/0042-*.'
  );
});
jest.mock('./create-auth', () => {
  throw new Error(
    '@lets-park/auth/client reached "./create-auth" — the server half. See doc/decision/0042-*.'
  );
});

describe('@lets-park/auth/client', () => {
