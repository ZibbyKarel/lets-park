 ...
 * once per request in `apps/web/src/app/layout.tsx` — which reads the
 * `NEXT_LOCALE` cookie and the `Accept-Language` header (see
 * `./locale.ts`) — and handed down through `apps/web/src/app/providers.tsx`.
 *
 * This lib deliberately does not import the catalogs. They live in
 * `apps/web/messages/*.json` (`TODO.md` item 9), and `libs/i18n` is
 ...
 * type crosses into `apps/web`'s props, and a wrapper lib that re-exported the
 ...
 * `AppConfig["Messages"]` augmentation in `apps/web/next-intl.d.ts`.
 */
