 ...
 * of the wire.
 *
 * `apps/api`'s `RealtimeIoAdapter` (`realtime-io.adapter.ts`) configures the
 * real `socket.io` `Server` with this value, and `libs/realtime-client`'s
 ...
 * `apps/api`'s own `SOCKET_IO_PATH` and the client's `DEFAULT_SOCKET_PATH` —
 * because `apps/api` (`scope:api`) cannot import `libs/realtime-client`
