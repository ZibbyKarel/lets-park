 ...
 * error enum.
 *
 * `libs/contract` defines the domain errors (`ERROR_CODES`, `errorShapeSchema`,
 ...
 * That flag is **always `false`** for this backend: `apps/api`'s global filter
...
export interface ContractError {
  code: ErrorCode;
  /** Developer-facing text. UI copy is keyed off `code` in `libs/i18n`. */
  message: string;
