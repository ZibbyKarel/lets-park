...
const LOCAL = 'postgresql://lets_park:pw@localhost:5432/lets_park';
const PRODUCTION = 'postgresql://lets_park:pw@prod.example.com:5432/lets_park';
...
describe('assertDisposableDatabase — hosts it accepts', () => {
  it.each([
    ['localhost', LOCAL],
    ['127.0.0.1', 'postgresql://lets_park:pw@127.0.0.1:5432/lets_park'],
    ['an IPv6 loopback literal', 'postgresql://lets_park:pw@[::1]:5432/lets_park'],
    ['a Compose service name', 'postgresql://lets_park:pw@postgres:5432/lets_park'],
    ['a Compose network alias', 'postgresql://lets_park:pw@lets-park-postgres:5432/lets_park'],
...
describe('assertDisposableDatabase — hosts it refuses', () => {
  it.each([
    ...
    ['an RDS endpoint', 'postgresql://u:pw@db.eu-central-1.rds.amazonaws.com:5432/lets_park'],
    ['a bare public IP', 'postgresql://u:pw@203.0.113.10:5432/lets_park'],
    ['a routable IPv6 address', 'postgresql://u:pw@[2001:db8::1]:5432/lets_park'],
  ...
  it('refuses a string that is not a URL at all, without echoing it', () => {
    expect(() => assertDisposableDatabase('lets_park@@@not a url', NO_OVERRIDE)).toThrow(
