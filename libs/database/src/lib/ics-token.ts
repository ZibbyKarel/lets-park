 ...
 *
 * `apps/api/src/auth/auth-user.service.ts` mints a real token as
 ...
 *
 * This is duplicated rather than imported because `libs/database` is a lib and
 * `apps/api` is an application: the dependency only runs the other way. Three
 ...
 * `apps/api/src/auth/auth-user.service.ts`.
 */
