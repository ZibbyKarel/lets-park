 ...
 * asks for `@lets-park/calendar-export`.
 *
 ...
 * `YYYY-MM-DD` already happened in `apps/lets-park/api`.
 */
 ...
 * boundary rule forbids `apps/lets-park/api` from reaching it — see
 * `doc/decision/0082-*`.
 */
...
export function buildReservationCalendar(feed: IcsFeed): string {
  const calendar = icalGenerator({
    ...
    // `-//lets-park//parking//CS`. Identifies the producer, as RFC 5545 §3.7.3
    // requires; it is not a hostname and nothing resolves it.
    prodId: { company: 'lets-park', product: 'parking', language: 'CS' },
...
function addEvent(calendar: ReturnType<typeof icalGenerator>, entry: IcsCalendarEntry): void {
  // `toUtcMidnight` is `libs/lets-park/shared-types`' one construction of the
