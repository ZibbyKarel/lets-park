 ...
 * in the values `apps/lets-park/web` passes in, which is the rule Task 11 has to hold to
...
export interface AuthOptions {
  /** `AUTH_OKTA_ISSUER`. Validated in `apps/lets-park/web/src/env.ts`. */
   ...
   * its built-in one at `/api/auth/signin`. `apps/lets-park/web` (Task 23) supplies the
   * real route.
   */
   ...
   * `apps/lets-park/web/src/env.ts`'s back; `true` is a property of the deployment
 ...
 * token does, because `libs/lets-park/realtime-client` has to put it in a Socket.io
  ...
  return {
    ...
    providers: [
      Okta({
        ...
        // one schema (`apps/lets-park/web/src/env.ts`), so nothing should be read from
