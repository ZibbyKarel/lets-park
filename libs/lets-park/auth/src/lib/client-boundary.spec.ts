...
jest.mock('next-auth', () => {
  throw new Error(
    '@lets-park/auth/client reached "next-auth" — the server half. See doc/decision/0046-*.'
  );
});
jest.mock('next-auth/providers/okta', () => {
  throw new Error(
    '@lets-park/auth/client reached "next-auth/providers/okta" — the server half. See doc/decision/0046-*.'
  );
});
jest.mock('./config', () => {
  throw new Error(
    '@lets-park/auth/client reached "./config" — the server half. Move what it needed into src/lib/session.ts. See doc/decision/0046-*.'
  );
});
jest.mock('./create-auth', () => {
  throw new Error(
    '@lets-park/auth/client reached "./create-auth" — the server half. See doc/decision/0046-*.'
  );
});
