/**
 * The Socket.io connection, typed from `@lets-park/contract/realtime`.
 *
 ...
 * the reason `libs/lets-park/auth`'s `useAccessTokenProvider` returns a function rather
 * than a string.
 *
 ...
 * `SOCKET_IO_PATH` — the one constant `apps/lets-park/api`'s `RealtimeIoAdapter` and this
 ...
 * guarantee this project can enforce from `libs/lets-park/realtime-client` — the other
 * half is `apps/lets-park/api/src/realtime/realtime-io.adapter.spec.ts`.
 */
...
export interface RealtimeSocketOptions {
  ...
  /**
   * Supplies the access token for the handshake. `libs/lets-park/auth`'s
   * `useAccessTokenProvider()` is the real implementation; the type is
   * imported from `@lets-park/api-client` rather than redeclared so that the
