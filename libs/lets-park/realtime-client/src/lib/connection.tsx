 ...
 *   tree can reach the same connection. It is what `apps/lets-park/web` renders
 ...
 * directive: `apps/lets-park/web` marks its own provider boundary as a client component
 ...
 * backgrounded, `libs/lets-park/auth`'s rotation had not run yet — and rebuilding the
 ...
 * consumer of `useRealtime()` in `apps/lets-park/web` destructures `status` and
...
export interface RealtimeConnectionOptions {
  ...
  /** Where the handshake token comes from — `libs/lets-park/auth`'s provider. */
   ...
   * either. `libs/lets-park/realtime-client` is the mandatory wrapper for
...
export function useRealtimeConnection(options: RealtimeConnectionOptions): RealtimeInternals {
  ...
  // Written during render on purpose, the same way `libs/lets-park/auth`'s
