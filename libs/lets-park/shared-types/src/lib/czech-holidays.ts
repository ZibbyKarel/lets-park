 ...
 * `buildMonthGrid` in `apps/lets-park/web/src/lot/bulk-modal/bulk-view.ts` (28–31 days
 * per month), by `planDay` in `apps/lets-park/api/src/reservations/bulk-allocator.ts`
 * (once per date in a bulk request), and by `isReservableDay` in
 * `apps/lets-park/api/src/overview/day-overview.service.ts`. Without this, walking a
