 ...
 * ```
 *
 * `apps/lets-park/web-e2e`'s `globalSetup` runs it (as a subprocess) before the browsers
 * start. Two things make that a subprocess rather than an import:
 *
 * 1. `libs/lets-park/database` is tagged `scope:api` and `apps/lets-park/web-e2e` is `scope:web`,
