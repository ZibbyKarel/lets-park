 ...
 *
 * `apps/lets-park/api/src/auth/auth-user.service.ts` mints a real token as
 ...
 *
 * This is duplicated rather than imported because `libs/lets-park/database` is a lib and
 * `apps/lets-park/api` is an application: the dependency only runs the other way. Three
 ...
 * `apps/lets-park/api/src/auth/auth-user.service.ts`.
 */
