 ...
 *
 * `libs/lets-park/shared-types` is `layer:foundation` with `allowedExternalImports: []`
 ...
 * `dateOnlySchema`; `libs/lets-park/contract` would not want a second validation path.
 ...
 *
 * `libs/lets-park/contract` is the only project that may import both: `type:contract`
...
import { isDateOnly, isYearMonth } from '@lets-park/shared-types';
