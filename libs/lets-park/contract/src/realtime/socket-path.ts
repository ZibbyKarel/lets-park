 ...
 * of the wire.
 *
 * `apps/lets-park/api`'s `RealtimeIoAdapter` (`realtime-io.adapter.ts`) configures the
 * real `socket.io` `Server` with this value, and `libs/lets-park/realtime-client`'s
 ...
 * `apps/lets-park/api`'s own `SOCKET_IO_PATH` and the client's `DEFAULT_SOCKET_PATH` —
 * because `apps/lets-park/api` (`scope:api`) cannot import `libs/lets-park/realtime-client`
 ...
 * runs.
 *
 * `@lets-park/contract/realtime` is `scope:shared` and both halves already
