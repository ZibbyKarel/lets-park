/**
 * Proof that `@lets-park/contract/realtime` is a genuinely separate entry point.
 *
...
describe('@lets-park/contract/realtime', () => {
  ...
  it('depends on exactly zod and the foundation lib', () => {
    ...
    expect(realtime.externals).toEqual(['@lets-park/shared-types', 'zod']);
  });

  it('is not re-exported from the root entry point', () => {
    // Two entry points, not one barrel with a realtime section: importing
    // `@lets-park/contract` must not pull the socket contract in either.
