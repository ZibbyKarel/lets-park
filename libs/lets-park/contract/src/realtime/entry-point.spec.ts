 ...
 * `tsconfig.base.json` is what turns it into `@lets-park/contract/realtime`, and
 * a missing or misspelt alias would first surface in Task 15 or Task 24.
 *
 * This file cannot simply `import '@lets-park/contract/realtime'`: a project may
...
describe('@lets-park/contract/realtime', () => {
  it('is declared as a path alias of its own', () => {
    expect(paths['@lets-park/contract/realtime']).toEqual([
      './libs/lets-park/contract/src/realtime/index.ts',
    ]);
  });

  it('points at a file that exists and is this module', () => {
    const target = resolve(WORKSPACE_ROOT, paths['@lets-park/contract/realtime']?.[0] as string);
  ...
  it('is a second entry point, not a replacement for the first', () => {
    expect(paths['@lets-park/contract']).toEqual(['./libs/lets-park/contract/src/index.ts']);
  });
