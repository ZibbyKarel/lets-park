 ...
 * URL**, so that `apps/lets-park/api` (which serves it) and `apps/lets-park/web` (which shows it)
 ...
 * data handed to `@lets-park/calendar-export` is domain data crossing a module
 * boundary, and contract-first applies to it for the same reason it applies to
 * everything else: `apps/lets-park/api` reads the rows, the calendar lib renders them, and
 ...
 * (`app.setGlobalPrefix('api')` in `apps/lets-park/api/src/main.ts`).
 */
